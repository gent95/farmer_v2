...
public class AFarmerController {
	...
	public Map listFarmer(String userId, Farmer farmer) {
		...
		try {
			...
			if (user != null) {
				...
				String[] propertys = new String[] { "id", "name", "addr", "farmlandNumber", "plantVariety", "lng",
						"lat" };
	...
	public Map getFarmer(String id) {
		...
		try {
			...
			list.add(maps);
			/*
			 * Farmland farmland = new Farmland(); List data = new ArrayList();
			 * farmland.setFarmer(farmer); List<Farmland> farmlandList =
			 * farmlandService.findList(farmland); String[] propert2 = new
			 * String[]{"id", "alias",
			 * "plantVaritety","usedId","usedName","assigneTime","area","addr",
			 * "nodeNumber","farmlandNum","landType","user","relay","remarks"};
			 * for (Farmland fa : farmlandList) { Map message =
			 * Maps.newHashMap(); for (String po : propert2) { message.put(po,
			 * Reflections.invokeGetter(fa, po)); } data.add(message); }
