...
//        List<Farmer> farmers = farmerService.findList(new Farmer());
    ...
    public Map getDatectionDates() {
        ...
        try {
            ...
            for (int i = datections.size() - 1; i > -1; i--) {
                winSpeed.add(datections.get(i).getWindSpeed());
                airTemperature.add(datections.get(i).getAirTemperature());
                humidity.add(datections.get(i).getHumidity());
                rainV.add(datections.get(i).getRainV());
