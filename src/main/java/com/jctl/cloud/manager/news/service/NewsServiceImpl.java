...
public class NewsServiceImpl implements INewsService, JCTLConfig {
    ...
    public List<News> newsData(HttpServletRequest request) {
        ...
        CacheUtils.put("request", request);
        ...
        //增加缓存
        list = getElement(request);
        CacheUtils.put("news", list);
        return list;
    }


    public List<News> getElement(HttpServletRequest request) {
        List<News> lists = new ArrayList<>();
        Document doc = JsoupUtils.getDocument(BURL, request);
